...
from src.data.utils import get_data_path_for_day # To create mock file paths
...
@pytest.fixture
def mock_kline_df():
    """Provides a valid mock kline DataFrame (1h interval for a day)."""
    start_time = datetime(2024, 5, 1, 0, 0, 0, tzinfo=timezone.utc)
    # Last candle opens at 23:00:00 for a 1h interval spanning to end of day
...
def mock_cache_dir_for_integrity(tmp_path):
    """Creates a temporary cache directory for integrity tests."""
...
def test_parse_filename_agg_trades():
    ...
    assert metadata['end_time_utc'] == datetime(2024, 5, 1, 23, 59, 59, 999999, tzinfo=timezone.utc) # End of day
    assert 'interval' not in metadata # Should not have interval
...
def test_parse_filename_kline():
    ...
    assert set(metadata['features']) == set(['Close', 'RSI_14', 'SMA_20', 'Open', 'High', 'Low', 'Volume']) # Base OHLCV added
...
def test_validate_daily_data_agg_trades_valid(mock_agg_trades_df, mock_cache_dir_for_integrity):
    """Test valid aggregate trades file."""
...
def test_validate_daily_data_wrong_dtype(mock_agg_trades_df, mock_cache_dir_for_integrity):
    """Test file with incorrect column data type."""
    filepath = get_data_path_for_day('2024-05-01', 'BTCUSDT', 'agg_trades', cache_dir=str(mock_cache_dir_for_integrity))
    df_bad = mock_agg_trades_df.copy()
    df_bad['Price'] = df_bad['Price'].astype(str) # Make it string
    df_bad.to_parquet(filepath)
    is_valid, msg = validate_daily_data(filepath)
    assert not is_valid
...
def test_validate_daily_data_non_monotonic_index(mock_agg_trades_df, mock_cache_dir_for_integrity):
    """Test file with non-monotonic index."""
    filepath = get_data_path_for_day('2024-05-01', 'BTCUSDT', 'agg_trades', cache_dir=str(mock_cache_dir_for_integrity))
    df_bad = mock_agg_trades_df.iloc[[0, 2, 1]].copy() # Shuffle index
    df_bad.to_parquet(filepath)
    is_valid, msg = validate_daily_data(filepath)
    assert not is_valid
...
def test_validate_daily_data_timestamp_range_error(mock_agg_trades_df, mock_cache_dir_for_integrity):
    """Test timestamp range validation for agg_trades."""
    filepath = get_data_path_for_day('2024-05-01', 'BTCUSDT', 'agg_trades', cache_dir=str(mock_cache_dir_for_integrity))
    
    # Create DataFrame for a different day
    df_bad_time = mock_agg_trades_df.copy()
    df_bad_time.index = df_bad_time.index + timedelta(days=5) # Shift to 2024-05-06
    df_bad_time.to_parquet(filepath) # But file is for 2024-05-01
...
def test_validate_daily_data_kline_interval_consistency(mock_kline_df, mock_cache_dir_for_integrity):
    ...
    df_bad_interval = mock_kline_df.iloc[::2].copy() # Skip every other candle
