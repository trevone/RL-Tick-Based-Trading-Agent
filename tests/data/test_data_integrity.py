...
# These are the core utilities being tested or used as helpers in this test file.
from src.data.check_tick_cache import (
    parse_filename_for_metadata,  # Function to extract metadata from cached file names
    validate_daily_data,          # Main function to validate the content and structure of data files
    interval_to_timedelta         # Helper for converting Binance interval strings to timedelta objects
)
from src.data.utils import get_data_path_for_day # Helper to construct expected file paths for cached data


# Apply pytest-order marker to ensure this module runs first
# This helps establish foundational data integrity before testing components that rely on data.
pytestmark = pytest.mark.order(1)


# --- Fixtures for common test data ---
# Fixtures provide reusable data or setup routines for multiple tests.

@pytest.fixture
def mock_agg_trades_df():
    """
    Provides a valid mock aggregate trades DataFrame.
    This DataFrame simulates real tick data with price, quantity, and buyer maker information,
    spanning a full day with millisecond-frequency timestamps.
    """
...
def mock_kline_df():
    ...
    This DataFrame simulates K-line data (Open, High, Low, Close, Volume)
    along with mock technical indicator features like SMA and RSI.
    """
    start_time = datetime(2024, 5, 1, 0, 0, 0, tzinfo=timezone.utc)
...
def mock_cache_dir_for_integrity(tmp_path):
    ...
    This ensures tests are isolated and don't interfere with actual cached data.
    """
    return tmp_path / "integrity_cache"

# --- Tests for parse_filename_for_metadata ---
# These tests verify that file names, generated by the data caching utility,
# can be correctly parsed to extract embedded metadata (e.g., data type, symbol, date).
...
# These tests verify the robustness of the validate_daily_data function,
# checking for various conditions that indicate corrupted, incomplete, or malformed data files.
