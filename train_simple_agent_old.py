...
from stable_baselines3.common.vec_env import VecNormalize # Added this import
import os
import pandas as pd
import numpy as np
import json
...
def train_agent(
    ...
    try:
        def env_fn():
            ...
            # Apply Monitor here so each individual environment created by make_vec_env is monitored
            monitored_env = Monitor(wrapped_env, filename=os.path.join(log_dir, "monitor.csv") if log_to_file else None)
            return monitored_env

        # make_vec_env will create a vectorized environment.
        # If n_envs > 1, it will create multiple instances of monitored_env.
        vec_env = make_vec_env(env_fn, n_envs=1, seed=0)

        # Now, apply VecNormalize to the vectorized environment (vec_env)
    ...
    try:
        ...
        # Create a vectorized evaluation environment using the same env_fn
        eval_vec_env = make_vec_env(env_fn, n_envs=1, seed=0) 
        # Apply VecNormalize to the vectorized evaluation environment
